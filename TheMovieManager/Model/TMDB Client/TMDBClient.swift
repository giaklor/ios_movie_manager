...
class TMDBClient {
    ...
    class func getSessionId(completion: @escaping (Bool, Error?) -> Void) {
        ...
        taskForPOSTRequest(url: Endpoints.getSession.url, responseType: SessionResponse.self, body: body) {
            (response, error) in
            if let response = response {
    ...
    class func getRequestToken(completion: @escaping (Bool, Error?) -> Void) {
        taskForGETRequest(url: Endpoints.getRequestToken.url, responseType: RequestTokenResponse.self) {
            (response, error) in
            if let response = response {
                Auth.requestToken = response.requestToken
                completion(true, nil)
